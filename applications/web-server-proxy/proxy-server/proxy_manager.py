...
class ProxyManager:
    ...
    def add_admin(self, email, passw):
        ...
        :return: VOID
        """
        admin_dic = {'email': email, 'passw': passw}
        proxy_admin_list = self.proxy_admins
        if email not in proxy_admin_list:
            proxy_admin_list.append(admin_dic)
        else:
            print("Duplicated email account detected")
        return 0
    ...
    def list_of_admins(self):
        ...
        """
        admin_list = []
        if not all(elem in self.proxy_admins for elem in admin_list):
            admin_list.append(self.proxy_admins)
        return admin_list
